...
author: "Samuel Dion & Oleg Kartavtsev"
date: '2022-06-04'
output: 
...
### Pre-Processing
```{r}
drop <- ("examiner_art_unit3")
df_filter1 <- drop_na(df_filter, clo_centrality)
df_filter1 <- drop_na(df_filter1, tenure_days)
#df_filter1 <- df_filter1[!is.na(df_filter1$app_proc_time)]
df_filter1 <- drop_na(df_filter1, app_proc_time)
#df_filter1 = df_filter1[!is.na(df_filter1$bet_centrality)]
df_filter1 <- select(df_filter1, -drop)
```

### Creating centralities/graph
```{r}
g_w <- graph_from_data_frame(edges_filter, directed=TRUE)
# degree centrality
degree <- degree(g_w)
# betweenness centrality
betweenness <- betweenness(g_w)
# closeness centrality
closeness <- closeness(g_w)
# eigenvector centrality
eigenvector <- eigen_centrality(g_w)$vector
centralities <- cbind(degree, closeness, eigenvector, betweenness)
centralities_df_ <- cbind(nodes, centralities)
head(centralities_df_)
```

### Joining 2 datasets

```{r}
join <- df_filter1[c(1,6,22,23,24)]
model_df = merge(x=centralities_df_,y=join,by="examiner_id")
model_df <- model_df %>% relocate(application_number)
```
...
correlationMatrix <- cor(model_df[,8:15])
...
First model - Application Processing Time

```{r}
model_everything <- lm(app_proc_time ~ gender + race + tenure_days + degree + closeness + eigenvector + betweenness + degree_in + degree_out, data=model_df)
summary(model_everything)
```

Second Model - Degree Out (Asking Advice)

```{r}
model_out <- lm(degree_out ~ app_proc_time + gender + race + tenure_days + closeness + eigenvector + degree_in, data=model_df)
summary(model_out)
```

Third Model - Degree In (Giving Advice)

```{r}
model_in <- lm(degree_in ~ app_proc_time + gender + race + tenure_days + closeness + eigenvector + degree_out, data=model_df)
summary(model_in)
```
...
```{r}
library(stargazer)
stargazer(model_everything, model_out, model_in, title="Results of the Regression Analysis", type = "latex",  dep.var.labels = c("Processing Time", "Asking Advice", "Giving Advice"), single.row = TRUE, align=TRUE)
```
