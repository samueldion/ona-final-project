...
```{r}
app_filter <- applications %>% drop_na(gender, race) #%>% filter(examiner_art_unit3 %in% c("162", "179")) 
...
ggraph(graph, layout = "kk") + 
    ...
    geom_node_point(aes(fill = examiner_art_unit3), colour = "#000000", size = 2, shape = 21, stroke = 0.2) + 
...
```
Removing the outliers is needed
...
```{r}
lm2.fit <- lm(data=df_filter, (app_proc_time)~degree_in+degree_out+bet_centrality+clo_centrality
              )
...
```{r}
lm3.fit <- lm(data=df_filter, (app_proc_time)~degree_in+degree_out+bet_centrality+clo_centrality+
...
## Regression Attempt
```{r}
df_filter$tenure_years <- df_filter$tenure_days/365
lm4.fit <- lm(data=df_filter, app_proc_time~tenure_years)
